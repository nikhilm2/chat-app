...
const SignUp = () => {
  ...
  return (
    <div className='flex flex-col items-center justify-center min-w-96 mx-auto'>
        ...
        bg-opacity-0'>
           <h1 className='text-3xl font-semibold text-center text-gray-300'> SignUp <span className=' text-blue-500'>TalkApp</span>
            </h1>
            <form onSubmit={handleSubmit}>
              ...
              <label className='label p-2'>
                ...
                <input type="password" placeholder="Confirm Password" className="input input-bordered w-full h-10"
                 value={inputs.confirmPassword} onChange={(e)=>setInput({...inputs,confirmPassword: e.target.value})}
                 />
              </div>

              <GenderCheckbox onCheckboxChange={handleCheckboxChange} selectedGender={inputs.gender}/>

              <Link to="/login" className=' hover:underline hover:text-blue-400 mt-2 inline-block'>
                Already have an Account?
              </Link>

              <div>
              <button className='btn btn-sm btn-block mt-2 border-slate-700' disabled={loading}>
